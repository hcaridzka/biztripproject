...
const receiptStatusLabel = (status?: string) => status === 'approved' ? 'APPROVED' : status === 'partial' ? 'PARTIAL' : status === 'rejected' ? 'REJECTED' : 'PENDING';
const isImageAttachment = (value?: string | null, name?: string | null) => !!value && (value.startsWith('data:image/') || /\.(png|jpg|jpeg|webp|gif)(\?|$)/i.test(value) || (!!name && /\.(png|jpg|jpeg|webp|gif)$/i.test(name)));
const isPdfAttachment = (value?: string | null, name?: string | null) => !!value && (value.startsWith('data:application/pdf') || /\.pdf(\?|$)/i.test(value) || (!!name && /\.pdf$/i.test(name)));

function splitMovementName(value: string) {
  const [name, ...componentParts] = String(value || '').split(' — ');
...
function Section({ title, children }: { title: string; children: ReactNode }) { return <section className="mt-5"><h2 className="mb-2 text-[11px] font-bold uppercase">{title}</h2>{children}</section>; }
function InfoGrid({ children }: { children: ReactNode }) { return <div className="grid grid-cols-2 gap-2 border p-3">{children}</div>; }
...
function Table({ headers, children }: { headers: string[]; children: ReactNode }) { return <table className="w-full border-collapse text-[10px]"><thead><tr>{headers.map((header) => <th key={header} className="border px-2 py-1.5 text-left">{header}</th>)}</tr></thead><tbody>{children}</tbody></table>; }
function TD({ children, colSpan }: { children: ReactNode; colSpan?: number }) { return <td colSpan={colSpan} className="border px-2 py-1.5 align-top">{children}</td>; }
