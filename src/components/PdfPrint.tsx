      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[95vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="no-print flex items-center justify-between px-6 py-3 border-b border-slate-100 sticky top-0 bg-white z-10">
          <h3 className="text-sm font-bold text-slate-800">
            {mode === 'advance' ? 'Cetak PDF Surat Perintah Perjalanan Dinas (SPD)' : 'Cetak PDF Laporan & Settlement SPD'}
          </h3>
          ...
          {mode === 'advance' ? (
            <>
              ...
              <table className="w-full mt-6 text-xs border border-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="py-2 px-3 text-left border border-slate-200">No</th>
                    <th className="py-2 px-3 text-left border border-slate-200">Tujuan</th>
                    <th className="py-2 px-3 text-left border border-slate-200">Tanggal</th>
                    <th className="py-2 px-3 text-left border border-slate-200">Agenda</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <table className="w-full mt-6 text-xs border border-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="py-2 px-3 text-left border border-slate-200">Komponen Biaya</th>
                    <th className="py-2 px-3 text-right border border-slate-200">Nominal (Rp)</th>
                  </tr>
                </thead>
                <tbody>
                  {trip.cost_data?.perParticipant?.map((pp: any, i: number) => (
                    <tr key={i}>
                      <td className="py-2 px-3 border border-slate-200">Tunjangan — {pp.name} ({pp.jabatan})</td>
                      <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(pp.total)}</td>
                    </tr>
                  ))}
                  {trip.cost_data?.perParticipant?.some((pp: any) => pp.hotel > 0) && (
                    <tr>
                      <td className="py-2 px-3 border border-slate-200">Akomodasi Hotel</td>
                      <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(trip.cost_data.perParticipant.reduce((s: number, p: any) => s + p.hotel, 0))}</td>
                    </tr>
                  )}
                  <tr>
                    <td className="py-2 px-3 border border-slate-200">BBM (Fuel)</td>
                    <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(Number(trip.fuel_cost) || 0)}</td>
                  </tr>
                  <tr>
                  ...
                  {extras.map((e) => (
                    <tr key={e.id}>
                      ...
                      <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(Number(e.nominal) || 0)}</td>
                    </tr>
                  ))}
                  <tr className="bg-brand-50 font-bold">
                    <td className="py-2 px-3 border border-slate-200">GRAND TOTAL ADVANCE</td>
                    <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(advance)}</td>
                  </tr>
                </tbody>
              </table>

              <div className="mt-6 rounded-xl bg-slate-50 ring-1 ring-slate-200 px-5 py-4 text-xs text-slate-700 space-y-1">
                <div className="font-bold text-sm text-slate-800 mb-1">Status Persetujuan</div>
                <div>Status: Approved / Ready for Trip</div>
                <div>Tanggal Pengajuan: {formatDate(trip.submitted_at)}</div>
                <div>Tanggal Disetujui: {formatDate(trip.approved_at)}</div>
              </div>
            </>
          ) : (
            ...
            <>
              <div className="mt-6 text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">Pertanggungjawaban Pengeluaran & Hasil Perjalanan Dinas</p>
              </div>
              ...
              <table className="w-full mt-6 text-xs border border-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="py-2 px-3 text-left border border-slate-200">Komponen</th>
                    <th className="py-2 px-3 text-right border border-slate-200">Nominal (Rp)</th>
                    <th className="py-2 px-3 text-left border border-slate-200">Status</th>
                    <th className="py-2 px-3 text-left border border-slate-200">Beban PT</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="py-2 px-3 border border-slate-200">Total Advance Diterima</td>
                    <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(advance)}</td>
                    <td className="py-2 px-3 border border-slate-200">-</td>
                    <td className="py-2 px-3 border border-slate-200">-</td>
                  </tr>
                  <tr>
                    <td className="py-2 px-3 border border-slate-200">Total Realisasi Aktual</td>
                    <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(actual)}</td>
                    <td className="py-2 px-3 border border-slate-200">-</td>
                    <td className="py-2 px-3 border border-slate-200">-</td>
                  </tr>
                  {claims.map((c) => (
                    <tr key={c.id}>
                      <td className="py-2 px-3 border border-slate-200">{c.name}</td>
                      <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(Number(c.nominal) || 0)}</td>
                      <td className="py-2 px-3 border border-slate-200">{c.claim_status}</td>
                      <td className="py-2 px-3 border border-slate-200">{c.pt_burden}</td>
                    </tr>
                  ))}
                  <tr className="bg-brand-50 font-bold">
                    <td className="py-2 px-3 border border-slate-200">SELISIH ({diff > 0 ? 'REIMBURSE' : diff < 0 ? 'REFUND' : 'SETTLED'})</td>
                    <td className="py-2 px-3 text-right border border-slate-200">{formatIDR(Math.abs(diff))}</td>
                    <td className="py-2 px-3 border border-slate-200" colSpan={2}>{trip.settlement_result ?? '-'}</td>
                  </tr>
                </tbody>
              </table>

              <div className="mt-6 rounded-xl bg-slate-50 ring-1 ring-slate-200 px-5 py-4 text-xs text-slate-700 space-y-1">
                <div className="font-bold text-sm text-slate-800 mb-1">Status Penyelesaian</div>
                <div>Status: Settled</div>
                <div>Tanggal Pengajuan Settlement: {formatDate(trip.settlement_submitted_at)}</div>
                <div>Tanggal Disetujui: {formatDate(trip.settlement_reviewed_at)}</div>
              </div>
            </>
          )}
