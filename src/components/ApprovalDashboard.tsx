...
export function ApprovalDashboard({ setSelectedTrip, setView }: { setSelectedTrip: (id: string) => void; setView: (v: ViewKey) => void }) {
  ...
  const pendingBurdenPT = (t: BizTrip) => (t.company_burden ?? []).filter((pt) => !direksiApprovals(t)[pt]);
  ...
  const ptFilterMatches = (t: BizTrip) => !ptFilter || (t.status === 'Pending Manager Approval' ? t.requester_pt === ptFilter : (t.company_burden ?? []).includes(ptFilter));

  const queue = useMemo(() => {
    let list: BizTrip[] = [];
    if (role === 'Manager') list = trips.filter((t) => t.status === 'Pending Manager Approval' && managerPtMatches(t));
    else if (role === 'Direksi') list = trips.filter((t) => t.status === 'Pending Direksi Approval' && burdenPtMatches(t));
    else if (role === 'HR Manager') list = trips.filter((t) => ['Pending Manager Approval', 'Pending PIC Obligo', 'Pending Direksi Approval', 'Pending HR Advance Review'].includes(t.status));
  ...
  const ptFilterOptions = useMemo(() => {
    ...
    return Array.from(new Set(isSuperAdmin ? active : ptAccess)).sort();
  }, [activePTMaster, isSuperAdmin, ptAccess]);

  const handleDelete = async (id: string) => {
    if (!confirm('Apakah Anda yakin ingin menghapus pengajuan trip ini?')) return;
    try { await deleteTrip(id); showToast('success', 'Pengajuan trip berhasil dihapus'); if (selected?.id === id) setSelected(null); refresh(); }
    catch (e: any) { showToast('error', 'Gagal menghapus: ' + e.message); }
  };
  ...
  const approve = async (t: BizTrip) => {
    try {
      ...
      if (t.status === 'Pending Manager Approval') {
        ...
        if (approvable.length === 0) { showToast('error', 'Tidak ada PT Beban yang dapat Anda approve.'); return; }
        const approvals = { ...direksiApprovals(t) };
        const now = new Date().toISOString();
        approvable.forEach((pt) => { approvals[pt] = { approvedBy: profile?.name ?? '', approvedAt: now }; });
        const remaining = (t.company_burden ?? []).filter((pt) => !approvals[pt]);
        nextStatus = remaining.length === 0 ? 'Pending HR Advance Review' : 'Pending Direksi Approval';
        patch.cost_data = { ...(t.cost_data ?? {}), direksiApprovals: approvals };
        patch.direksi_approved_by = profile?.name ?? '';
        patch.direksi_approved_at = now;
        trackingRemark = `Approved PT: ${approvable.join(', ')}${remaining.length ? `. Menunggu: ${remaining.join(', ')}` : ''}`;
      ...
      if (nextStatus === t.status && t.status !== 'Pending Direksi Approval') { showToast('error', 'Status trip tidak dapat diproses melalui approval ini.'); return; }
      ...
      showToast('success', nextStatus === 'Pending Direksi Approval' ? 'Approval PT Anda tersimpan; masih menunggu Direksi PT lain.' : 'Pengajuan berhasil disetujui');
      refresh(); setSelected(null);
    } catch (e: any) { showToast('error', 'Gagal approve: ' + e.message); }
  };

  const doReject = async () => {
    if (!rejectTrip || !rejectReason.trim()) return showToast('error', 'Remarks alasan reject wajib diisi');
    try { await updateTrip(rejectTrip.id, { status: 'Rejected', reject_reason: rejectReason, reject_by: profile?.name ?? '', rejection_stage: rejectTrip.status }); await supabase.from('trip_tracking').insert({ trip_id: rejectTrip.id, actor_name: profile?.name ?? '', actor_role: role, action: 'Rejected', from_status: rejectTrip.status, to_status: 'Rejected', remarks: rejectReason }); showToast('success', 'Pengajuan ditolak dengan remarks'); setRejectTrip(null); setRejectReason(''); setSelected(null); refresh(); }
    catch (e: any) { showToast('error', 'Gagal reject: ' + e.message); }
  };

  const requestReReview = async (t: BizTrip) => {
    const justification = window.prompt('Tulis justifikasi permintaan Re-Review:'); if (!justification?.trim()) return;
    try { await updateTrip(t.id, { status: 'Pending Manager Approval', review_justification: justification, reject_reason: null, cost_data: { ...(t.cost_data ?? {}), direksiApprovals: {} } }); await supabase.from('trip_tracking').insert({ trip_id: t.id, actor_name: profile?.name ?? '', actor_role: role, action: 'Request Re-Review', from_status: t.status, to_status: 'Pending Manager Approval', remarks: justification }); showToast('success', 'Re-Review diajukan'); refresh(); }
  ...
  return <div className="space-y-6 animate-slide-up max-w-5xl mx-auto">
    ...
    {(role === 'Manager' || role === 'Direksi' || role === 'HR Manager') && ptFilterOptions.length > 0 && <Card className="p-4"><div className="flex items-center gap-3 flex-wrap"><div className="flex items-center gap-2 text-sm font-semibold text-slate-700"><Filter className="w-4 h-4 text-brand-600" />Pilih PT:</div><button onClick={() => setPtFilter('')} className={cn('px-3 py-1.5 rounded-xl text-xs font-semibold ring-1', !ptFilter ? 'bg-brand-600 text-white ring-brand-600' : 'bg-white ring-slate-200')}>Semua PT</button>{ptFilterOptions.map((pt) => <button key={pt} onClick={() => setPtFilter(pt)} className={cn('px-3 py-1.5 rounded-xl text-xs font-semibold ring-1', ptFilter === pt ? 'bg-brand-600 text-white ring-brand-600' : 'bg-white ring-slate-200')}>{pt}</button>)}</div><p className="text-[11px] text-slate-400 mt-2">Manager: PT Pemohon. Direksi: masing-masing PT Beban; multi-PT wajib selesai seluruh approval.</p></Card>}
    {queue.length === 0 ? <EmptyState icon={<CheckSquare className="w-6 h-6" />} title="Tidak ada approval menunggu" /> : <div className="space-y-3">{queue.map((t) => <Card key={t.id} className="p-5"><div className="flex flex-col md:flex-row md:items-center gap-4"><div className="flex-1 cursor-pointer" onClick={() => setSelected(t)}><div className="flex gap-2 flex-wrap"><strong>{t.requester_name}</strong><StatusBadge status={t.status} /></div><div className="text-xs text-slate-500 mt-1">{t.requester_pt || '-'} · {formatDate(t.departure_date)} s.d. {formatDate(t.return_date)}</div><div className="text-sm mt-1">{t.purpose}</div><div className="text-[11px] text-slate-500 mt-1">Beban: {(t.company_burden ?? []).join(', ') || '-'}{t.status === 'Pending Direksi Approval' ? ` · Pending: ${pendingBurdenPT(t).join(', ') || '-'}` : ''}</div></div><div className="flex gap-2 flex-wrap"><Button size="sm" variant="secondary" onClick={() => setSelected(t)}>Detail</Button>{renderPrimaryAction(t)}<Button size="sm" variant="danger" icon={<X className="w-3.5 h-3.5" />} onClick={() => { setRejectTrip(t); setRejectReason(''); }}>Reject</Button>{isSuperAdmin && <button onClick={() => handleDelete(t.id)} className="p-2 text-rose-400"><Trash2 className="w-4 h-4" /></button>}</div></div></Card>)}</div>}
    <Modal open={!!selected} onClose={() => setSelected(null)} title="Detail Pengajuan">{selected && <div className="space-y-4"><div className="grid md:grid-cols-2 gap-3 text-sm"><Info label="Pemohon" value={selected.requester_name} /><Info label="PT Pemohon" value={selected.requester_pt || '-'} /><Info label="NIP" value={selected.requester_nip || '-'} /><Info label="Jabatan" value={selected.requester_jabatan || '-'} /><Info label="Periode" value={`${formatDate(selected.departure_date)} s.d. ${formatDate(selected.return_date)}`} /><Info label="Beban Biaya" value={(selected.company_burden ?? []).join(', ') || '-'} /><Info label="Tujuan" value={selected.purpose} /><Info label="Estimasi" value={formatIDR(selected.cost_grand_total ?? 0)} /></div>{selected.status === 'Pending Direksi Approval' && <div className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800"><strong>Approval Direksi:</strong> {(selected.company_burden ?? []).map((pt) => `${pt}: ${direksiApprovals(selected)[pt] ? 'Approved' : 'Pending'}`).join(' · ')}</div>}<div className="flex justify-end gap-2">{renderPrimaryAction(selected)}<Button variant="danger" onClick={() => { setRejectTrip(selected); setRejectReason(''); }}>Reject</Button></div></div>}</Modal>
