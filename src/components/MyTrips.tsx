...
import { MapPin, FileText, RotateCcw, X, Play, CalendarClock } from 'lucide-react';
...
export function MyTrips({ onPrint, selectedTripId }: { onPrint: (id: string) => void; selectedTripId?: string | null }) {
  ...
  const requestReReview = async (t: BizTrip) => {
    ...
    showToast('success', 'Re-Review diajukan'); refresh();
  };
  ...
  const startTrip = async (t: BizTrip) => {
    ...
    showToast('success', 'Trip dimulai'); refresh();
  };
  ...
  const doCancel = async () => {
    ...
    try {
      ...
      showToast('success', 'Pengajuan berhasil dibatalkan'); setCancelTrip(null); setCancelReason(''); refresh();
    } catch (e: any) { showToast('error', 'Gagal membatalkan pengajuan: ' + e.message); }
  };
  ...
  const rescheduleTarget = (t: BizTrip): TripStatus => {
    // Perubahan tanggal tidak perlu mengulang approval yang sudah lewat. Tetap di stage aktif.
    // Jika sudah final approved, kembali ke HR agar advance/SPD direfresh.
    if (t.status === 'Approved / Ready for Trip') return 'Pending HR Advance Review';
    return t.status;
  };
  ...
  const doReschedule = async () => {
    ...
    try {
      await updateTrip(rescheduleTrip.id, {
        itinerary: shifted, departure_date: newDepartureDate, departure_time: newDepartureTime || null, return_date: newReturnDate, return_time: newReturnTime || null,
        total_days: daysBetween(newDepartureDate, newReturnDate), status: target, cancel_reason_category: 'Reschedule', cancel_reason_detail: rescheduleReason,
        review_justification: `Reschedule dari ${oldSchedule} menjadi ${newSchedule}. Alasan: ${rescheduleReason}`,
        ...(rescheduleTrip.status === 'Approved / Ready for Trip' ? { spd_number: null, spd_issued_at: null, approved_at: null } : {}),
      });
      ...
      setRescheduleTrip(null); setRescheduleReason(''); refresh();
    } catch (e: any) { showToast('error', 'Gagal mengajukan reschedule: ' + e.message); }
  };
  ...
  if (selected) return <div className="space-y-4 max-w-4xl mx-auto animate-slide-up"><div className="flex items-center justify-between"><Button variant="secondary" size="sm" onClick={() => setSelectedId(null)}>← Back</Button><div className="flex gap-2 flex-wrap">{selected.status === 'Approved / Ready for Trip' && <Button size="sm" icon={<Play className="w-3.5 h-3.5" />} onClick={() => startTrip(selected)}>Start Trip</Button>}{canReschedule(selected) && <Button size="sm" variant="secondary" icon={<CalendarClock className="w-3.5 h-3.5" />} onClick={() => openReschedule(selected)}>Reschedule</Button>}{canCancel(selected) && <Button size="sm" variant="danger" icon={<X className="w-3.5 h-3.5" />} onClick={() => { setCancelTrip(selected); setCancelReason(''); }}>Cancel Trip</Button>}</div></div><TripDetail trip={selected} onPrint={onPrint} />{selected.status === 'Rejected' && <Card className="p-4"><Button size="sm" variant="secondary" icon={<RotateCcw className="w-3.5 h-3.5" />} onClick={() => requestReReview(selected)}>Request Re-Review</Button></Card>}</div>;

  return <div className="space-y-6 animate-slide-up max-w-4xl mx-auto">
    ...
    {myTrips.length === 0 ? <Card className="p-6"><EmptyState icon={<MapPin className="w-6 h-6" />} title="Belum ada trip" /></Card> : <div className="space-y-2">{myTrips.map((t) => <Card key={t.id} className="p-4 hover:ring-brand-200 transition"><div className="flex items-start justify-between gap-4 cursor-pointer" onClick={() => setSelectedId(t.id)}><div className="flex-1"><div className="text-sm font-semibold">{t.purpose}</div><div className="text-xs text-slate-400 mt-1">{t.origin} → {t.itinerary?.[0]?.destination ?? '-'} · {formatDate(t.departure_date)} · {daysBetween(t.departure_date, t.return_date)} hari · {formatIDR(Number(t.cost_grand_total) || 0)}</div></div><StatusBadge status={t.status} /></div><div className="mt-3 flex gap-2 flex-wrap">{t.status === 'Approved / Ready for Trip' && <Button size="sm" icon={<Play className="w-3.5 h-3.5" />} onClick={(e) => { e.stopPropagation(); startTrip(t); }}>Start Trip</Button>}{t.status === 'Rejected' && <Button size="sm" variant="secondary" icon={<RotateCcw className="w-3.5 h-3.5" />} onClick={(e) => { e.stopPropagation(); requestReReview(t); }}>Re-Review</Button>}{['Approved / Ready for Trip', 'On Trip', 'Completed'].includes(t.status) && <Button size="sm" variant="secondary" icon={<FileText className="w-3.5 h-3.5" />} onClick={(e) => { e.stopPropagation(); onPrint(t.id); }}>Surat Perjalanan Dinas</Button>}{canReschedule(t) && <Button size="sm" variant="secondary" icon={<CalendarClock className="w-3.5 h-3.5" />} onClick={(e) => { e.stopPropagation(); openReschedule(t); }}>Reschedule</Button>}{canCancel(t) && <Button size="sm" variant="danger" icon={<X className="w-3.5 h-3.5" />} onClick={(e) => { e.stopPropagation(); setCancelTrip(t); setCancelReason(''); }}>Cancel Trip</Button>}</div></Card>)}</div>}

    <Modal open={!!cancelTrip} onClose={() => setCancelTrip(null)} title="Cancel Request"><div className="space-y-4"><Field label="Catatan Alasan Pembatalan" required><Textarea rows={3} value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} /></Field><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setCancelTrip(null)}>Batal</Button><Button variant="danger" onClick={doCancel}>Confirm Cancel</Button></div></div></Modal>
