...
import { AlertCircle, Building2, Calendar, Calculator, Clock, FilePlus, MapPin, Paperclip, Plus, Send, Trash2, Users } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useApp } from '../context/AppContext';
import { Button, Card, EmptyState, Field, Input, Select, Textarea, formatIDR } from './ui-shared';
import { ORIGINS, DESTINATION_OPTIONS, TRANSPORT_CHOICES, JABATAN_LEVELS } from '../lib/constants';
...
export function RequestForm({ onDone }: { onDone: () => void }) {
  ...
  const activeGrades = useMemo(
    () => travelMatrixRows.filter((row) => row.is_active && row.grade_key && row.grade_name),
    [travelMatrixRows]
  );

  const fallbackGrade = useMemo(() => {
    if (profile?.grade && activeGrades.some((row) => row.grade_key === profile.grade)) return profile.grade;
    const byName = activeGrades.find((row) => row.grade_name === profile?.grade);
    if (byName) return byName.grade_key;
    if (profile?.jabatan === 'Direksi') return activeGrades.find((row) => row.grade_key === 'Direksi')?.grade_key ?? activeGrades[0]?.grade_key ?? '';
    if (profile?.jabatan === 'General Manager') return activeGrades.find((row) => row.grade_key === 'GM')?.grade_key ?? activeGrades[0]?.grade_key ?? '';
    if (profile?.jabatan === 'Head Department' || profile?.jabatan === 'Team Leader') return activeGrades.find((row) => row.grade_key === 'Head/TL')?.grade_key ?? activeGrades[0]?.grade_key ?? '';
    return activeGrades.find((row) => row.grade_key === 'Staff')?.grade_key ?? activeGrades[0]?.grade_key ?? '';
  ...
  const { depDate, retDate, depTime, retTime, days } = useMemo(() => {
    ...
    const sorted = [...itinerary].sort((a, b) => a.start_date.localeCompare(b.start_date));
    const first = sorted[0];
    const last = [...itinerary].sort((a, b) => a.end_date.localeCompare(b.end_date)).at(-1)!;
    return {
      depDate: first.start_date,
      retDate: last.end_date,
      depTime: first.start_time || '08:00',
      retTime: last.end_time || '17:00',
  ...
  const mainApplicant = useMemo<Participant>(() => ({
    ...
    category: 'Internal',
    pt_unit: applicantPt,
  }), [profile?.name, jabatan, grade, applicantPt]);
  ...
  const validation = useMemo(() => {
    ...
    if (!grade) errors.push('Grade pemohon wajib dipilih');
    ...
    participants.forEach((p, i) => {
      ...
      if (p.jabatan === 'Driver') errors.push(`Partisipan tambahan baris ${i + 1}: Driver tidak boleh diinput sebagai participant. Checklist kebutuhan Driver dan tulis catatan bila diperlukan.`);
      if (p.category !== 'Eksternal' && !p.grade) errors.push(`Partisipan tambahan baris ${i + 1}: Grade wajib dipilih`);
  ...
  const preview = useMemo(() => {
    if (!itinerary.length || !depDate || !retDate) return null;
    return computeCost({
      ...
      totalDistance,
      matrix: travelMatrix,
      dkMatrix: travelDKMatrix,
      driverIncentive,
    });
  }, [allParticipants, days, itinerary, origin, tripCategory, kpScheme, needsDriver, totalDistance, depDate, retDate, travelMatrix, travelDKMatrix, driverIncentive]);
  ...
  const addItinerary = () => {
    ...
    const date = last?.end_date || new Date().toISOString().slice(0, 10);
    setItinerary((rows) => [...rows, { id: uid(), start_date: date, start_time: last?.end_time || '08:00', end_date: date, end_time: '17:00', destination: '', destination_custom: '', kpScheme: 'KP2', isWithinCity: false, isLuarkota: false, agenda: '' }]);
  };
  const updateItinerary = (id: string, patch: Partial<ItineraryLeg>) => setItinerary((rows) => rows.map((row) => {
  ...
  const uploadPettyFile = async (file: File) => {
    const ext = file.name.split('.').pop();
    const path = `approvals/petty_${profile?.id ?? 'user'}_${Date.now()}.${ext}`;
    const upload = await supabase.storage.from('pettycash').upload(path, file);
    if (upload.error) throw upload.error;
    return supabase.storage.from('pettycash').getPublicUrl(path).data.publicUrl;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast('error', `Gagal submit: ${e.message}`);
    } finally {
      setBusy(false);
    }
  };

  const jabatanOptions = JABATAN_LEVELS.filter((item) => item !== 'Driver');
  ...
  return <div className="space-y-6 animate-slide-up max-w-5xl mx-auto">
    ...
    <Card className="p-6 space-y-4">
      ...
      <div className="grid md:grid-cols-5 gap-4">
        ...
        <Field label="Jabatan" required><Select value={jabatan} onChange={(e) => setJabatan(e.target.value as Jabatan)}>{jabatanOptions.map((item) => <option key={item}>{item}</option>)}</Select></Field>
        <Field label="Grade Benefit" required><Select value={grade} onChange={(e) => setGrade(e.target.value)}><option value="">Pilih grade...</option>{activeGrades.map((row) => <option key={row.id} value={row.grade_key}>{row.grade_name} ({row.grade_key})</option>)}</Select></Field>
      </div>
      <div className="grid md:grid-cols-4 gap-4">
        <Field label="Transportasi"><Select value={needsVehicle} onChange={(e) => setNeedsVehicle(e.target.value as TransportChoice)}>{TRANSPORT_CHOICES.map((item) => <option key={item}>{item}</option>)}</Select></Field>
        <Field label="Butuh Driver?"><Select value={needsDriver ? 'ya' : 'tidak'} onChange={(e) => setNeedsDriver(e.target.value === 'ya')}><option value="tidak">Tidak</option><option value="ya">Ya</option></Select></Field>
        <Field label="Total Distance"><Select value={totalDistance} onChange={(e) => setTotalDistance(e.target.value as TotalDistanceOption)}><option value="none">Kurang dari 200 km</option><option value="gt200">&gt; 200 km</option><option value="gt400">&gt; 400 km</option></Select></Field>
        <Field label="Tujuan Perjalanan" required><Input value={purpose} onChange={(e) => setPurpose(e.target.value)} /></Field>
      </div>
      <div className="rounded-xl bg-blue-50 ring-1 ring-blue-200 p-3 text-xs text-blue-700"><strong>Driver tidak diinput sebagai participant.</strong> Pilih kebutuhan Driver di atas. PIC Obligo akan menentukan Driver; kebutuhan khusus dapat ditulis pada Remarks.</div>
    ...
    <Card className="p-6 space-y-4">
      <h3 className="text-sm font-bold flex items-center gap-2"><MapPin className="w-4 h-4" /> Lokasi Asal & Beban Biaya</h3>
      <Field label="Origin"><Select value={origin} onChange={(e) => setOrigin(e.target.value)}>{ORIGINS.map((item) => <option key={item}>{item}</option>)}</Select></Field>
      {origin === 'Others' && <Field label="Nama Origin"><Input value={originCustom} onChange={(e) => setOriginCustom(e.target.value)} /></Field>}
      <div><div className="text-xs font-semibold mb-2">PT Beban Biaya</div><div className="flex flex-wrap gap-2">{activePTMaster.map((row) => <button type="button" key={row.id} onClick={() => toggleBurden(row.name)} className={cn('px-3 py-1.5 rounded-xl text-xs font-semibold ring-1', companyBurdens.includes(row.name) ? 'bg-brand-600 text-white ring-brand-600' : 'bg-white ring-slate-200')}>{row.name}</button>)}</div></div>
    </Card>

    <Card className="p-6 space-y-4">
      <div className="flex justify-between"><div><h3 className="text-sm font-bold flex items-center gap-2"><MapPin className="w-4 h-4" /> Itinerary</h3><p className="text-[11px] text-slate-400">Tanggal perjalanan dihitung dari itinerary.</p></div><Button size="sm" variant="secondary" icon={<Plus className="w-3.5 h-3.5" />} onClick={addItinerary}>Add Row</Button></div>
      {!itinerary.length ? <EmptyState title="Belum ada itinerary" /> : <div className="space-y-3">{itinerary.map((leg, i) => { const tiers = dkTiersForOrigin(origin); return <div key={leg.id} className="rounded-xl ring-1 ring-slate-200 p-4 space-y-3"><div className="flex justify-between"><span className="text-xs font-bold flex gap-1"><Clock className="w-3.5 h-3.5" /> Leg {i + 1}</span><button onClick={() => removeItinerary(leg.id)}><Trash2 className="w-4 h-4 text-rose-500" /></button></div><div className="grid md:grid-cols-4 gap-3"><Input type="date" value={leg.start_date} onChange={(e) => updateItinerary(leg.id, { start_date: e.target.value })} /><Input type="time" value={leg.start_time} onChange={(e) => updateItinerary(leg.id, { start_time: e.target.value })} /><Input type="date" value={leg.end_date} onChange={(e) => updateItinerary(leg.id, { end_date: e.target.value })} /><Input type="time" value={leg.end_time} onChange={(e) => updateItinerary(leg.id, { end_time: e.target.value })} /></div><div className="grid md:grid-cols-2 gap-3"><Select value={leg.destination} onChange={(e) => { const dest = e.target.value; updateItinerary(leg.id, { destination: dest, isWithinCity: dest === 'Dalam Kota', isLuarkota: dest === 'Luar Kota' }); }}><option value="">Pilih lokasi...</option>{DESTINATION_OPTIONS.map((item) => <option key={item}>{item}</option>)}</Select>{(leg.destination === 'Others' || leg.destination === 'Luar Kota' || leg.destination === 'Dalam Kota') && <Input value={leg.destination_custom ?? ''} onChange={(e) => updateItinerary(leg.id, { destination_custom: e.target.value })} placeholder="Nama lokasi/kota" />}</div>{leg.isWithinCity && <Select value={leg.dkTier ?? ''} onChange={(e) => updateItinerary(leg.id, { dkTier: e.target.value as DKTier })}><option value="">Pilih tier DK...</option>{tiers.map((tier) => <option key={tier.key} value={tier.key}>{tier.label}</option>)}</Select>}<Input value={leg.agenda} onChange={(e) => updateItinerary(leg.id, { agenda: e.target.value })} placeholder="Agenda" /></div>; })}</div>}
    </Card>

    <Card className="p-6 space-y-4">
      <div className="flex justify-between"><div><h3 className="text-sm font-bold flex items-center gap-2"><Users className="w-4 h-4" /> Partisipan Tambahan</h3><p className="text-[11px] text-slate-400">Driver tidak ditambahkan di sini.</p></div><Button size="sm" variant="secondary" icon={<Plus className="w-3.5 h-3.5" />} onClick={addParticipant}>Add Row</Button></div>
      {participants.map((p, i) => { const cat: ParticipantCategory = p.category ?? 'Internal'; return <div key={p.id} className="grid md:grid-cols-[40px_120px_1fr_160px_160px_160px_40px] gap-2 items-center border rounded-xl p-3"><span>{i + 1}.</span><Select value={cat} onChange={(e) => updateParticipant(p.id, { category: e.target.value as ParticipantCategory })}><option>Internal</option><option>Eksternal</option></Select><Input value={p.name} onChange={(e) => updateParticipant(p.id, { name: e.target.value })} placeholder="Nama" />{cat === 'Internal' ? <><Select value={p.pt_unit ?? ''} onChange={(e) => updateParticipant(p.id, { pt_unit: e.target.value })}><option value="">PT...</option>{ptOptions.map((pt) => <option key={pt}>{pt}</option>)}</Select><Select value={p.jabatan} onChange={(e) => updateParticipant(p.id, { jabatan: e.target.value as Jabatan })}>{jabatanOptions.map((item) => <option key={item}>{item}</option>)}</Select><Select value={p.grade ?? ''} onChange={(e) => updateParticipant(p.id, { grade: e.target.value })}><option value="">Grade...</option>{activeGrades.map((row) => <option key={row.id} value={row.grade_key}>{row.grade_name}</option>)}</Select></> : <><Input className="md:col-span-3" value={p.keterangan ?? ''} onChange={(e) => updateParticipant(p.id, { keterangan: e.target.value })} placeholder="Vendor / Klien / Mitra" /></>}<button onClick={() => setParticipants((rows) => rows.filter((row) => row.id !== p.id))}><Trash2 className="w-4 h-4 text-rose-500" /></button></div>; })}
    </Card>
    ...
    {preview && <Card className="p-6 space-y-3"><h3 className="text-sm font-bold">Estimasi Awal</h3>{preview.perParticipant.map((pp) => <div key={`${pp.name}-${pp.grade}`} className="rounded-xl bg-slate-50 p-3 flex justify-between"><div><strong>{pp.name}</strong><div className="text-xs text-slate-500">{pp.jabatan} · Grade {pp.grade || '-'} · {pp.breakdown}</div></div><strong>{formatIDR(pp.total + pp.hotel + pp.pettyCash)}</strong></div>)}<div className="border-t pt-3 flex justify-between"><strong>Total Estimasi</strong><strong>{formatIDR(preview.grandTotal)}</strong></div></Card>}
