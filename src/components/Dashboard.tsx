...
import { MapPin, Clock, CheckCircle2, TrendingUp, Calendar, ArrowRight } from 'lucide-react';
...
export function Dashboard({
  ...
  // Data trip berdasarkan role (Employee = milik sendiri, Manager/HR/Finance = seluruh trip)
  const displayTrips = useMemo(() => {
    return isEmployee ? trips.filter((t) => t.user_id === profile?.id) : trips;
  }, [trips, profile, isEmployee]);

  const activeTrips = useMemo(() => {
    return displayTrips.filter((t) => !['Completed', 'Rejected'].includes(t.status));
  }, [displayTrips]);

  const stats = useMemo(() => {
    const total = displayTrips.length;
    const active = activeTrips.length;
    const completed = displayTrips.filter((t) => t.status === 'Completed').length;
    const totalAdvance = displayTrips.reduce((s, t) => s + Number(t.cost_grand_total || 0), 0);
    return { total, active, completed, totalAdvance };
  }, [displayTrips, activeTrips]);

  // Pipeline distribution (keseluruhan alur kerja sistem)
    ...
    <div className="space-y-6 animate-slide-up max-w-6xl mx-auto">
      ...
      <Card className="p-6">
        <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-slate-400" /> Pipeline Overview
        </h3>
              ...
              <div key={step.status} className="flex items-center gap-2 shrink-0">
                <div
                  className={`px-3 py-2 rounded-xl text-xs font-semibold ${
                    count > 0 ? 'bg-brand-50 text-brand-700 ring-1 ring-brand-200' : 'bg-slate-50 text-slate-400 ring-1 ring-slate-100'
                  }`}
                >
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-slate-800">
            {isEmployee ? 'My Recent Trips' : 'Recent Trip Requests'}
          </h3>
          {isEmployee ? (
            <button
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-slate-800 truncate">
                    {t.purpose} {t.requester_name ? `(${t.requester_name})` : ''}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    {t.origin} → {t.itinerary?.[0]?.destination ?? '-'} · {formatDate(t.departure_date)} ·{' '}
                    {daysBetween(t.departure_date, t.return_date)} hari
                  </div>
                </div>
...
function StatCard({
  ...
  label: string;
  value: string;
  color: 'brand' | 'amber' | 'emerald' | 'slate';
}) {
