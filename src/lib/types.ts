...
export interface BizTrip {
  id: string; created_at: string; user_id: string; requester_name: string; requester_nip: string | null; requester_jabatan: Jabatan; requester_pt?: string | null;
  origin: string; origin_custom: string | null; departure_date: string; departure_time: string | null; return_date: string; return_time: string | null; total_days: number; purpose: string;
  needs_vehicle: boolean; vehicle_type_choice: string | null; needs_driver: boolean; requires_vehicle?: boolean; requires_driver?: boolean; total_distance?: TotalDistanceOption;
  itinerary: ItineraryLeg[]; participants: Participant[]; petty_cash_requested: boolean; petty_cash_approval_file: string | null; petty_cash_holder: string | null; company_burden: string[]; trip_category: TripCategory; kp_scheme: KPScheme; status: TripStatus;
  rejection_stage: string | null; reject_reason: string | null; reject_by: string | null; review_justification: string | null;
  manager_note: string | null; manager_approved_by: string | null; manager_approved_at: string | null;
  obligo_vehicle_type: string | null; obligo_vehicle_plate: string | null; obligo_vehicle_km: string | null; obligo_driver_name: string | null; obligo_note: string | null; obligo_approved_by: string | null; obligo_approved_at: string | null;
  assigned_vehicle_id: string | null; assigned_driver_id: string | null; vehicle_km: number | null; fuel_cost: number; etoll_cost: number;
  direksi_note: string | null; direksi_approved_by: string | null; direksi_approved_at: string | null;
  cost_data: any; cost_grand_total: number; cost_fuel: number; cost_toll: number; cost_note: string | null; hr_notes: string | null; spd_number: string | null; spd_issued_at: string | null; approved_at: string | null; submitted_at: string;
  work_result: string | null; pending_task: string | null; next_project: string | null; settlement_submitted_by: string | null; settlement_submitted_at: string | null; settlement_number: string | null; realization_total: number | null; approved_total: number | null; settlement_result: string | null; settlement_note: string | null; settlement_reviewed_by: string | null; settlement_reviewed_at: string | null; completed_at: string | null;
  cancel_reason_category: string | null; cancel_reason_detail: string | null; banding_reason: string | null; banding_at: string | null; employee_remarks: string | null;
}

export interface AllowanceMatrix { grade_key: string; luar_kota: number; kp1: number; kp2: number; kpo: number; hotel: number; petty_cash: number; dk_25: number; dk_50: number; dk_100: number; driver_incentive_gt200: number; driver_incentive_gt400: number; }
